import React, { useState } from 'react'
import './Jobcard.css';
import Onejob from './Onejob';

// import location from '../asse'
export default function Jobcard(props) {
    const [one, setOne] = useState(false);
    const [read, Setread] = useState(false);

    // const navigate = useNavigate();
    function viewHandler() {
        setOne(true);
        Setread(true);
        console.log("clicked");
        console.log("props output", props);
        // return <Onejob title={props.title} key={props.index} desc={props.description} minsal={props.maxsal} company={props.company} maxsal={props.minsal} place={props.place} />
    }

    function hideHandler() {
        setOne(false);
        Setread(false);
    }
    return (
        <>
            <div className='jobmaindiv21'>
                <div className='jobstyle21'>
                    <div style={{ display: "flex" }}>
                        <h1 className='jobtitle21' >{props.title.split(" ").slice(0, 5).join(" ") + "..."}</h1>

                        {/* <div><button>add</button></div> */}
                    </div>

                    <div className='location21'>
                        <p className='company21' >{props.company}</p>
                        <div style={{ display: 'flex', gap: "4px" }}>
                            <div><img style={{ width: "16px", height: "16px", mixBlendMode: "multiply", borderRadius: "20px", marginTop: "2px" }} src="data:image/png;base64," alt="location" /></div>
                            <div><p className='place21'>{props.place}</p></div>
                        </div>
                    </div>
                    <p className='desc21'>{props.desc.split(" ").slice(0, 8).join(" ") + "..."}</p>
                    <h3 className='salary21'>

                        {props.minsal === null && props.maxsal === null ? (<h3>Not avialable</h3>)
                            :
                            (<h3>₹ {props.minsal}k - {props.maxsal}k</h3>)
                        }
                    </h3>
                    <div className='apply21'>
                        <div style={{ display: "flex", gap: "15px", marginTop: "13px" }}>
                            <div><button className='applybutton21'>Fulltime</button></div>
                            <div><button className='applybutton21'>Remote</button></div>
                        </div>
                        <div>
                            {
                                read == false ?
                                    (<button className='viewbutton21' onClick={viewHandler}>View now</button>)
                                    :
                                    (<button className='viewbutton21' onClick={hideHandler}>Hide</button>)
                            }
                        </div>
                    </div>
                </div>
            </div>
            {
                one && read && (<Onejob title={props.title} key={props.index} desc={props.desc} minsal={props.maxsal} company={props.company} maxsal={props.minsal} place={props.place} publish={props.publish} type={props.type} jlink={props.jlink} />)
            }
        </>
    )
}
