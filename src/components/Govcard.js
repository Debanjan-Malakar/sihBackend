import React from 'react'
import './Govcard.css';
import Govonejob from './Govonejob';
import { useState } from 'react';
export default function Govcard(props) {
    const [one, setOne] = useState(false);
    const [read, Setread] = useState(false);
    function viewHandler() {
        setOne(true);
        Setread(true);
        console.log("clicked");
        console.log("props output", props);
        // return <Onejob title={props.title} key={props.index} desc={props.description} minsal={props.maxsal} company={props.company} maxsal={props.minsal} place={props.place} />
    }

    function hideHandler() {
        setOne(false);
        Setread(false);
    }
    return (
        <>
            <div className='gjobmaindiv21'>
                <div className='gjobstyle21'>
                    <div style={{ display: "flex" }}>
                        <h1 className='gjobtitle21' >{props.title.split(" ").slice(0, 5).join(" ") + "..."}</h1>

                        {/* <div><button>add</button></div> */}
                    </div>

                    <div className='glocation21'>
                        <p className='gcompany21' >{props.company}</p>
                        <div style={{ display: 'flex', gap: "4px" }}>
                            <div><img style={{ width: "16px", height: "16px", mixBlendMode: "multiply", borderRadius: "20px", marginTop: "2px" }} src="data:image/png;base64," alt="location" /></div>
                            <div><p className='gplace21'>{props.place}</p></div>
                        </div>
                    </div>
                    <p className='gdesc21'>{props.desc.split(" ").slice(0, 8).join(" ") + "..."}</p>
                    <h3 className='gsalary21'>
                           ₹ {props.minsal} - {props.maxsal}</h3>
                    <div className='gapply21'>
                        <div style={{ display: "flex", gap: "15px", marginTop: "13px" }}>
                            <div><button className='gapplybutton21'>Fulltime</button></div>
                            <div><button className='gapplybutton21'>Remote</button></div>
                        </div>
                        <div>
                            {
                                read == false ?
                                    (<button className='gviewbutton21' onClick={viewHandler}>View now</button>)
                                    :
                                    (<button className='gviewbutton21' onClick={hideHandler}>Hide</button>)
                            }
                        </div>
                    </div>
                </div>
            </div>
            {
                one && read && (<Govonejob title={props.title} key={props.index} desc={props.desc} minsal={props.maxsal} company={props.company} maxsal={props.minsal} place={props.place} publish={props.publish} type={props.type} jlink={props.jlink} />)
            }
        </>
    )
}
